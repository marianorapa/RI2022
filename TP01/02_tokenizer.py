...
def is_abbreviation(token, collection):
    ...
    return bool(re.match(f"{regex_1}|{regex_2}|{regex_3}", token)) and not exists_without_last_period
    ...
    for in_file in corpus_path.iterdir():
        ...
        with open(in_file, "r", encoding="utf-8") as f:
            ...
            for line in f.readlines():
                ...
                tokens_list =  list(filter(lambda x: x not in palabras_vacias and len(x) >= MIN_LENGTH and len(x) <= MAX_LENGTH,
                                     [translate(x) for x in line.strip().split()]))                                
