        ...
        with open(in_file, "r", encoding="latin1") as f:            
            for line in f.readlines():                
                ...
                for i in range(0, len(line)-1):
                    if (line[i] == " "): continue
                    combination = f"{line[i]}{line[i+1]}"                    
                    if combination in models[in_file.name]:
                        models[in_file.name][combination] += 1
                    else:
                        models[in_file.name][combination] = 1    
    ...
    with open(path, "r", encoding="latin1") as f:
        for line in f.readlines():
            ...
            for i in range(0, len(line)-1):
                if (line[i] == " "): continue
                combination = f"{line[i]}{line[i+1]}"                    
                if combination in test_model:
                    test_model[combination] += 1
                else:
                    test_model[combination] = 1                                            
...
def compare_solutions(solutions, lang_detect_results, prediction):
...
if __name__ == '__main__':    
    ...
    if (len(sys.argv)) == 4:
        ...
        compare_solutions(solutions, lang_detect_results, prediction)
