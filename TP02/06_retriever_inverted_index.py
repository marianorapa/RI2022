...
def get_abbreviations(token, vocabulary):    
    ...
    if (re.match("[A-Za-z]+\.", token)):
        exists_without_last_period = token[:-1] in vocabulary       # if it ends with period, checks if it's in the collection without it
    if (re.match("[A-Z]+", token)):
        exists_lower = token.lower() in vocabulary
...
def get_tokens_and_freq_in_line(line, tokens_freq):
    ...
    for token in tokens_list:                
        ...
        if (len(numbers) > 0):
            ...
            abbreviations = get_abbreviations(token, tokens_freq)
...
def index_doc(doc_id, doc, index, docs_terms):
    tokens_freq = {}
    try:
        with open(doc, "r", encoding="utf-8") as f:                
            for line in f.readlines():                 
                get_tokens_and_freq_in_line(line, tokens_freq)                

            for token in tokens_freq.keys():
                if token not in index:
                    index[token] = [0]
                # Acumulate TF in array's first position   
                index[token][0] += tokens_freq[token]    
                index[token].append([doc_id, tokens_freq[token]])
...
def index_files(files):        
    ...
    for file in files:
        ...
        index_doc(doc_id, current_file, index, doc_terms)
        doc_id += 1
...
def new_calculate_term_weight_in_query(term, frequency, index):
    # (1+log(Fiq)) * log(N/ni)    
    if (term in index):
        return (1 + frequency) * math.log(len(index)/index[term][0])
    return 0
...
def new_calculate_query_weights(query_tokens, index):
    weights = {}    
    for token in query_tokens:
        weights[token] = new_calculate_term_weight_in_query(token, query_tokens[token], index)
...
def new_calculate_doc_weights_tfidf(doc_terms, index): 
    weights = {}
    for term in doc_terms:
        tfidf = (1 + math.log(doc_terms[term])) * math.log(len(index)/index[term][0])
        weights[term] = tfidf
...
def new_sim(query, index, doc_terms):
    query_weights = new_calculate_query_weights(query, index)
    doc_weights = new_calculate_doc_weights_tfidf(doc_terms, index)    
    return cosine_distance(query_weights, doc_weights)

def retrieve_docs(query, index, docs_terms):
    scores = {}
    # The index has term frequencies in each doc. TF-IDF is pending
    query_terms = {}
    get_tokens_and_freq_in_line(query, query_terms)
    for term in query_terms:
        ...
        if (term in index):
            ...
            for posting in postings[1:]:
                print(posting)                
                doc_id = posting[0]
                scores[doc_id] = new_sim(query_terms, index, docs_terms[doc_id])    
    return scores
...
def save_index(dict):
    path = "output_06_inverted/index.pcl"
...
def save_docs_terms(doc_terms):
    path = "output_06_inverted/doc_terms.pcl"
    with open(path, "wb") as f:                
...
if __name__ == '__main__':
    ...
    if not os.path.exists("./output_06_inverted"):
        os.mkdir("./output_06_inverted")
    else:
        try:
            with open("output_06_inverted/index.pcl", "rb") as f:
                index = pickle.load(f)            
            with open("output_06_inverted/docs_terms.pcl", "rb") as f:
                docs_terms = pickle.load(f)                            
            index_loaded = True
            print("Found index at output_06_inverted. Delete the folder to index again.")
        except:
    ...
    if not index_loaded:
        ...
        index, docs_terms = index_files(files)
        end = time.time()    
        print("\r\nIndexing time: {} seconds.".format(end - start))
        save_index(index)
        save_docs_terms(docs_terms)
    ...
    if queries_file:
        ...
        for query_key in queries.keys():
            retrieved_docs = retrieve_docs(queries[query_key], index, docs_terms)
        ...
        while True:
            ...
            retrieved_docs = retrieve_docs(query, index, docs_terms)
            print("--------- Results ---------")        
            ordered = dict(sorted(retrieved_docs.items(), key=lambda item: item[1], reverse=True))
